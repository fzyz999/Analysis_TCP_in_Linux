...
%----------------------------------------------------------------------------------------
        ...
        \subsection{第二次握手：发送SYN+ACK段}
            ...
            \subsubsection{基本调用关系}            
                \begin{figure}[htb]        
                   \center{\includegraphics[width=\textwidth]  {images/Server:Send SYN+ACK.png}}
...
\end{minted}
                ...
                首先为将要发送的数据申请发送缓存，如果没有申请到，那就会返回NULL。这里有一个\mintinline{C}{unlikely}优化分析，参见appendix(to do by ref).
...
\end{minted}
        ...
        \subsection{第三次握手：接收ACK段}
			...
			\subsubsection{基本调用关系}
				\begin{figure}[htb]        
					\center{\includegraphics[width=\textwidth]  {images/Server:Receive ACK.png}}
